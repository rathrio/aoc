def pad(pair)
  left, right = pair
  return pair if left.size == right.size

  left = left.clone
  right = right.clone

  diff = (left.size - right.size).abs
  left.size > right.size ? diff.times { right << nil } : diff.times { left << nil }
...
def part1
  ...
  pairs.each_with_index do |pair, i|
    index = i + 1
    sum += index if compare(pair).negative?
  end
