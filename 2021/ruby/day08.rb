...
INPUT = DATA
  ...
  .map(&:chomp)
  .map { |line| line.split(' | ') }
  .map { |signals, output| { signals: signals.split, output: output.split } }

def part1
  unique_counts = [2, 4, 3, 7].to_set

  count = INPUT.map do |input|
    output = input[:output]
    output.count { |digit| unique_counts.include? digit.size }
  end.sum

  puts count
end

def one?(segment, known)
  segment.size == 2
end

def two?(segment, known)
  return false if known.key?(2)
  return true if segment.size == 5 && known.values_at(3, 5).compact.size == 2

  chars = segment.chars.to_set

  subset_exl = known.values_at(1, 4, 7).compact
  c1 = !subset_exl.empty? && subset_exl.none? { |other| chars >= other.chars.to_set  }

  superset_exl = known.values_at(6, 9).compact
  c2 = !superset_exl.empty? && superset_exl.none? { |other| chars <= other.chars.to_set }

  segment.size == 5 && c1 && c2
end

def three?(segment, known)
  return false if known.key?(3)
  return true if segment.size == 5 && known.values_at(2, 5).compact.size == 2

  chars = segment.chars.to_set

  subset_inc = known.values_at(1).compact
  c1 = !subset_inc.empty? && subset_inc.all? { |other| chars >= other.chars.to_set  }

  segment.size == 5 && c1
end

def four?(segment, known)
  segment.size == 4
end

def five?(segment, known)
  return false if known.key?(5)
  return true if segment.size == 5 && known.values_at(2, 3).compact.size == 2

  chars = segment.chars.to_set

  subset_exl = known.values_at(1, 4, 7).compact
  c1 = subset_exl.size == 3 && subset_exl.none? { |other| chars >= other.chars.to_set  }

  superset_inc = known.values_at(6, 9).compact
  c2 = !superset_inc.empty? && superset_inc.all? { |other| chars <= other.chars.to_set }

  segment.size == 5 && c1 && c2
end

def six?(segment, known)
  return false if known.key?(6)
  return true if segment.size == 6 && known.values_at(9, 0).compact.size == 2

  chars = segment.chars.to_set

  subset_inc = known.values_at(5).compact
  c1 = !subset_inc.empty? && subset_inc.all? { |other| chars >= other.chars.to_set  }

  subset_exl = known.values_at(1, 3, 4, 7).compact
  c2 = !subset_exl.empty? && subset_exl.none? { |other| chars >= other.chars.to_set }

  segment.size == 6 && c1 && c2
end

def seven?(segment, known)
  segment.size == 3
end

def eight?(segment, known)
  segment.size == 7
end

def nine?(segment, known)
  return false if known.key?(9)
  return true if segment.size == 6 && known.values_at(6, 0).compact.size == 2

  chars = segment.chars.to_set

  subset_inc = known.values_at(3, 4).compact
  c1 = !subset_inc.empty? && subset_inc.all? { |other| chars >= other.chars.to_set  }

  segment.size == 6 && c1
end

def zero?(segment, known)
  return false if known.key?(0)
  return true if segment.size == 6 && known.values_at(6, 9).compact.size == 2

  chars = segment.chars.to_set

  subset_inc = known.values_at(1, 7).compact
  c1 = subset_inc.size == 2 && subset_inc.all? { |other| chars >= other.chars.to_set  }

  subset_exl = known.values_at(3, 4, 5).compact
  c2 = subset_exl.size == 3 && subset_exl.none? { |other| chars >= other.chars.to_set }

  segment.size == 6 && c1 && c2
end

def part2
  output_values = INPUT.map do |input|
    ...
    loop do
      break if known.size == 10

      to_delete = []
      s.each do |segment|
        case
        when one?(segment, known)
          known[1] = segment
          to_delete << segment
        when two?(segment, known)
          known[2] = segment
          to_delete << segment
        when three?(segment, known)
          known[3] = segment
          to_delete << segment
        when four?(segment, known)
          known[4] = segment
          to_delete << segment
        when five?(segment, known)
          known[5] = segment
          to_delete << segment
        when six?(segment, known)
          known[6] = segment
          to_delete << segment
        when seven?(segment, known)
          known[7] = segment
          to_delete << segment
        when eight?(segment, known)
          known[8] = segment
          to_delete << segment
        when nine?(segment, known)
          known[9] = segment
          to_delete << segment
        when zero?(segment, known)
          known[0] = segment
          to_delete << segment
        end
      end

      to_delete.each { |delete| s.delete(delete) }
    end

    segment_to_digit = known.map { |k, v| [v.chars.to_set, k] }.to_h
    output.map { |segment| segment_to_digit[segment.chars.to_set] }.join('').to_i
  end
