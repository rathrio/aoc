...
def invalid_part2?(id)
  ...
  id.chars.each_with_index do |char, index|
    longest_subsequence << char

    factor = id.length / longest_subsequence.length
    return false if factor == 1
    return true if longest_subsequence * factor == id
  end
...
def part2
  ...
  DATA.read.split(',').map(&:strip).reject(&:empty?).each do |range_str|
    x, y = range_str.split('-').map(&:to_i)
    range = x..y
    sum += range.select { |id| invalid_part2?(id) }.sum
  end
